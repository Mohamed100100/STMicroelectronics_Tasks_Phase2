 ...
 * AND TO BE HONST , I ASKED FROM CHATGPT TO SOLVE GENERATE THIS FUNCTION :) 
 *
...
void get_the_content_of_path(const char *path, char **content) {
    ...
    /* Open the directory */
        ...
        *content = malloc(BUFFER_SIZE); 
	memset(*content,'\0',BUFFER_SIZE);
	*content = strdup("The Directory Content : <br> <br>");
        struct dirent *entry;
    ...
    /* -------------------------- REGULAR FILES PART ------------------------- */
        ...
        if (!(path_stat.st_mode & S_IXUSR)) {
            ...
            *content = malloc(BUFFER_SIZE); 
            memset(*content,'\0',BUFFER_SIZE);
            *content = strdup("The File Content : <br> <br>");
        ...
        /* --------------------------- CGI FILES PART --------------------------- */
            ...
            *content = malloc(BUFFER_SIZE); 
	     memset(*content,'\0',BUFFER_SIZE);
	     *content = strdup("The OUTPUT is : <br> <br>");
            char buffer[BUFFER_SIZE] = {'\0'};
